...
// Returns all card names from the catalog for autocomplete / browsing
export function getAllCardNames() {
  return CARD_REWARDS.map(c => ({
    name: c.keywords[0]
      .split(' ')
...
export function searchCardCatalog(query) {
  if (!query || query.length < 2) return []
  const lower = query.toLowerCase()
  return CARD_REWARDS
    .filter(c => c.keywords.some(k => k.includes(lower)))
    .slice(0, 6)
    .map(c => ({
      name: c.keywords[0]
        .split(' ')

...
export function generateRecommendations(cards, taps) {
  ...
  // If still no spend data at all, use default monthly estimates for a college student
  const hasAnySpend = Object.values(spendByCategory).some(v => v > 0)
  if (!hasAnySpend) {
    spendByCategory.dining = 200
    spendByCategory.grocery = 100
    spendByCategory.streaming = 30
    spendByCategory.retail = 80
    spendByCategory.other = 50
  }

...
export function generateRecommendations(cards, taps) {
  ...
  // Fill in default monthly spend for any category the user hasn't tapped yet
  // so we can still recommend cards that cover spending gaps
  const DEFAULTS = { dining: 200, grocery: 150, streaming: 40, gas: 60, retail: 100, travel: 50, hotel: 30, other: 50 }
  for (const cat of CATEGORIES) {
    if (!spendByCategory[cat]) spendByCategory[cat] = DEFAULTS[cat]
  }
  ...
  const scored = RECOMMENDABLE_CARDS
    ...
    .map(rec => {
      ...
      for (const cat of CATEGORIES) {
        ...
        if (annualCatGain < 0.50) continue
